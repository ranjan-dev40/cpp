...
int numsDelayed = 0, totalDelay = 0, bt = 0, avgCust = 0, systemBusy = 0;
...
int main()
{
    ...
    while (numsDelayed < n)
    {
        ...
        avgCust += (serverStatus + timesOfArrival.size()) * (clck - lastTime);
        bt += serverStatus * (clck - lastTime);
        lastTime = clck;
    }
    cout << "Average number of customers in the system: " << 1.0 * avgCust / clck
         << endl;
