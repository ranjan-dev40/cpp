...
int main()
{
    ...
    for (int i = 1; i < num_customers; ++i)
    {
        ...
        customers[i].departure_time =
            max(customers[i - 1].departure_time, customers[i].arrival_time) + service_time;
    }
    for (const auto &customer : customers)
    {
        cout << "Arrival time: " << customer.arrival_time << ", Service time: " << customer.service_time
